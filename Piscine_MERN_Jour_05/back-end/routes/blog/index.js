...
router.get('/:login', async (req, res) => {
  ...
  const blogs = await Blog.find({user: user._id}).populate('user').populate('comments').populate('categories');
  res.json(blogs);
});
...
router.get('/:login/:id', async (req, res) => {
  ...
  const blog = await Blog.findOne({_id}).populate('user').populate('comments.user').populate('categories');
  res.json(blog);
});
...
router.post('/:login/:id/comment', async (req, res) => {
  ...
  res.json(await Blog.findOne({_id: blogId}).populate('user').populate('comments.user').populate('categories'));
});
...
router.delete('/:login/:id/comment/:commentId', async (req, res) => {
  ...
  const blogDelete = await Blog.findOne({_id: blogId}).populate('user').populate('comments').populate('categories');
